...
class DecisionIntelligence:
    ...
    def _needs_ai_analysis(self, context: Dict[str, Any]) -> bool:
        """Check if situation benefits from AI analysis"""
        return (
    ...
    def _hybrid_decision(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ai_analysis = self.gemini.analyze_complex_situation(situation_data)
            
            # Combine insights
            strategic_guidance = (
                f"AI Assessment: {ai_analysis.get('primary_concern', 'Monitoring')}. "
                f"Actions: {', '.join(ai_analysis.get('immediate_actions', [])[:2])}."
            )
